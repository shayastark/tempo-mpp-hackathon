...
export function EscrowCard({ escrowId }: { escrowId: bigint }) {
  ...
  return (
    <Link href={`/escrow?id=${escrowId.toString()}`}>
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 hover:border-zinc-600 transition-colors cursor-pointer">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <h3 className="font-semibold mt-1">
              {data.description || "Untitled Escrow"}
            </h3>
          </div>
          <span
